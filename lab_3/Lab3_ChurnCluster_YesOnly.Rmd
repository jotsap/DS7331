...
## Lab 3 - Clustering Part A: Yes Only
NOTE: this does clustering on ONLY customer from the Churn = Yes category
...
###ONLY Customers who have Churned

As this is the second exercise to ONLY look at customers who have churned, we will filter to only have those in our data set



```{r}

#filter on churn == yes customers

churn.df[churn.df$Churn == "Yes",] -> churn.df
...
To do more advanced visualization we will split the data into 2 separate dataframes: numeric and categorical. 

For exploratory sake we will also include the categorical variable InternetService, as our SME says this is a critical area to examine. 
...
churnNum.df <- churn.df %>% dplyr::select(tenure, MonthlyCharges, TotalCharges, InternetService)
...
### BOXPLOT OF NUMERICAL VALUES VS INTERNET SERVICE
boxplot_vs_response <- function(x) {
  ggplot(data = churnNum.df, aes_string(y=x, x="InternetService", fill="InternetService")) + geom_boxplot() + coord_flip() 
}
...
Next lets look at a barplots for the categoric variales to get a sense of the different ratio of customers who have internet service for each categorical column's level.


**NOTE** Again, showing ONLY customers who have churned, most of them are fiber customers, who are single, not seniors, on a month-to-month contract
...
churnCat.df <- churn.df %>% dplyr::select(-tenure, -MonthlyCharges, -TotalCharges, -Churn)
...
barplot_vs_response <- function(x) {
  ggplot(data = churnCat.df, aes_string(x = x, y="InternetService", fill = "InternetService")) + geom_col() 
}
...
hist(churn.df$tenure,
     main = "Churn = Yes Customer Tenure",
     xlab = "Months",
     ylab = "Customers")
...
plot(churn.df$monthlyCat,
     main = "Monthly Spend for Churn = Yes Customers",
     ylab = "Customers")

boxplot(churn.df$tenure ~ churn.df$monthlyCat,
        main = "Tenure by Monthly Cost Categories",
        horizontal = T,
        xlab = "Contract Months" )


boxplot(churn.df$MonthlyCharges ~ churn.df$tenureCat,
        main = "Monthly Charges by Tenure Categories",
        horizontal = T,
        xlab = "Dollars per Month" )
...
**Examining Tenure Categories Across Other Variables**

Lets see the monthly categories of Low, Med, and High vs the other categorical variables.
...
### BARPLOT OF CATEGORICAL VALUES VS TENURECAT

barplot_vs_response <- function(x) {
  ggplot(data = churnCat.df, aes_string(x = x, y="tenureCat", fill = "tenureCat")) + geom_col() 
}
...
**Examining Monthly Charges Categories Across Other Variables**
...
churnCatNames <- names(churnCat.df)
#remove Churn response
churnCatNames[-18] -> churnCatNames

### BARPLOT OF CATEGORICAL VALUES VS MONTHLYCAT

barplot_vs_response <- function(x) {
  ggplot(data = churnCat.df, aes_string(x = x, y="monthlyCat", fill = "monthlyCat")) + geom_col() 
}
...
Quickly examining the customer profiles, we can see of the 1869 customers who have internet, 243 stream TV but do *NOT* stream movies. And 247 of them stream movies, but not TV. So in both cases around 13% only stream one or the other.
...
Given the sharp change, 5 clusters appears to be the best choice
...
The silhouette coefficient ranges between -1 and 1, with 1 indicating good consistency within clusters, -1 — not so good. From the plot below, 5 clusters would be preferred
...
From the plot below, 3 and 5 clusters would be preferred
...
dendro.col <- dendro %>%
  set("branches_k_color", k = 5, value =   c("blue", "red", "green", "black", "orange")) %>%
...
cutree(churn.hclust.agg, k = 5) -> churn.agg.cutree

*second create div vector from cuttree*
cutree(churn.hclust.div, k = 5) -> churn.div.cutree
...
write.csv(churnout.df , file = "d:/temp/data/mining/final/churnyesout.csv", eol = "\n", row.names = T, append = F)
